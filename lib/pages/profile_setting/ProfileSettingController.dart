...
class ProfileSettingController extends GetxController {
  ...
  Future<void> getLabelTextDetail() async {
    try {
      await StageProvider().getLabelTextDetail(serviceController.langId.value, profileSetting, serviceController.token).then((resp) async {
        if (resp['status']!= null && resp['status'] == "Success") {
          if(resp['data'] != null && resp['data']['profileSettingPage'] != null){
            labelTextDetail.addAll(resp['data']['profileSettingPage']);
          }
        }
      }, onError: (error) {
